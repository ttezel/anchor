...
function roll(data, checksums, chunkSize) {
    var results = []
      ...
      //  If we get a match, keep track of whether we are out of matched block
      , outsideMatched = true;
    ...
    for (; end <= length; start++, end++) {
        ...
        //update whether outside last matched block
        if(start === lastMatchedEnd || end === length) {
          outsideMatched = true;
        }
